<template>
    <div>
        ...
        >
            ...
            <template
                    v-for="col in ['id','username', 'bookname','price','status', 'adress', 'phone','cid']"
                    :slot="col"
            ...
            <template slot="operation_edit" slot-scope="text, record, index">
                <div class="editable-row-operations">
                    ...
                    <span v-else>
                         <a :disabled="editingKey !== ''" v-show="record.status === '0'"
                            @click="editNode(record)">接单</a>
                         <a :disabled="editingKey !== ''" v-show="record.status === '1'" @click="cancelOrder(record)">取消订单</a>
                    </span>
                </div>
            </template>
        </a-table>
...
<!--                </el-form-item>-->
                ...
                <el-form-item label="简介" :label-width="formLabelWidth" prop="abs">
                    <el-input type="textarea" v-model="form.abs" maxlength="200" show-word-limit
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            listOrder() {
                let _this = this
                this.$axios.get('/listorder').then(resp => {
            ...
            cancelOrder: function (item) {
                ...
                MessageBox.confirm('确认取消订单？', '提示', {
                    ...
                    //'username', 'bookname','price', 'adress', 'phone','cid'
                    // this.data.status = 1
